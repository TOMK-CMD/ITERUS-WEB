...
1. Pick the issue in Linear (MCP), set *In Progress*, branch `feat/<issue-id>-slug`.
...
4. Ask the `reviewer` subagent, fix Blocking items, open the PR from the template with the
   Vercel preview URL and screenshots; move the issue to *In Review*.
...
- Codex reads the `AGENTS.md` chain (repo root → nested directories, 32 KiB budget,
  `AGENTS.override.md` wins). PR review: enable *Code review* for the repo in Codex settings,
